...
2. It copied and marked "session_start" as a time index variable into the new sessions entity to indicate the beginning of a session. If we had not set make_time_index, then ``normalize entity`` would have mode a "first_transactions_time" in this entity. If we don't want this time index to be created, we can set ``make_time_index=False``.
