...
def load_feature_plots():
    ...
    agg_feat = ft.AggregationFeature(ft.IdentityFeature(es, 'sessions', 'session_id'),
                                     'customers', ft.primitives.Count)
    trans_feat = ft.TransformFeature(ft.IdentityFeature(es, 'customers', 'join_date'), ft.primitives.TimeSincePrevious)
    demo_feat = ft.AggregationFeature(ft.TransformFeature(ft.IdentityFeature(es, 'transactions', 'transaction_time'),
