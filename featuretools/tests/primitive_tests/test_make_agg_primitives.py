...
def test_default_value_make_agg_primitive(pd_mock_customer):
    ...
    feature_matrix2, features = ft.dfs(
        entityset=pd_mock_customer,
        target_dataframe_name="sessions",
    ...
    assert (
        feature_matrix["MEAN_SUNDAY(transactions.amount, transaction_time)"].values
        != feature_matrix2["MEAN_SUNDAY(transactions.amount, transaction_time)"].values
    ).any()
