...
from featuretools.primitives.base import AggregationPrimitive, make_agg_primitive
...
def test_init_and_name(es):
    ...
    if es.dataframe_type == Library.KOALAS.value:
        agg_primitives = [
            prim for prim in agg_primitives if Library.KOALAS in prim.compatibility
        ]
...
def test_make_three_most_common(pd_es):
    ...
    NMostCommoner = make_agg_primitive(
        function=pd_top3,
