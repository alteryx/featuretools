...
def test_return_types(es):
    ...
    if es.dataframe_type != Library.PANDAS.value:
        pytest.xfail(
            "Dask and Koalas EntitySets do not support the NMostCommon primitive"
        )
    dfs_obj = DeepFeatureSynthesis(
        target_dataframe_name="sessions",
        entityset=es,
...
def test_primitive_options(es):
    ...
    options = {
        "month": {"ignore_columns": {"customers": ["date_of_birth"]}},
    ...
    for f in features:
        ...
        if isinstance(f.primitive, Month):
            for identity_base in columns:
                assert not (
                    identity_base.dataframe_name == "customers"
                    and identity_base.get_name() == "date_of_birth"
                )
...
def test_primitive_options_multiple_inputs(es):
    if es.dataframe_type != Library.PANDAS.value:
        pytest.xfail(
            "Dask and Koalas EntitySets do not support various primitives used in this test"
        )
    too_many_options = {
