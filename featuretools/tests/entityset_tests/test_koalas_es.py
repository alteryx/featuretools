...
def test_add_dataframe_from_ks_df(pd_es):
    ...
    pd.testing.assert_frame_equal(
        cleaned_df, ks_es["log_ks"].to_pandas(), check_like=True
    )
...
def test_create_entityset_with_mixed_dataframe_types(pd_es, ks_es):
    ...
    with pytest.raises(
        ValueError, match=err_msg.format(type(ks_df), type(pd_es.dataframes[0]))
    ):
        pd_es.add_dataframe(dataframe_name="new_dataframe", dataframe=ks_df, index="id")

    # Test error is raised when trying to add pandas dataframe to entitset with existing ks dataframes
    with pytest.raises(
        ValueError, match=err_msg.format(type(df), type(ks_es.dataframes[0]))
    ):
        ks_es.add_dataframe(dataframe_name="new_dataframe", dataframe=df, index="id")
...
def test_add_last_time_indexes():
    ...
    pd.testing.assert_series_equal(
        ...
        ks_es["sessions"][ks_lti_name].to_pandas().sort_index(),
        check_names=False,
    )
