...
def describe_feature(feature, feature_descriptions={}, primitive_templates={},
    ...
    if metadata_file:
        file_feature_descriptions, file_primitive_templates = parse_metadata_json(metadata_file)
...
def generate_description(feature, metadata=None):
    ...
    if isinstance(feature, ft.DirectFeature):
        return get_direct_description(feature, metadata)
    ...
    for input_col in input_columns:
        if input_col in feature_descriptions or input_col.unique_name() in feature_descriptions:
            ...
            if isinstance(input_col, ft.IdentityFeature):
                ...
                col_description = generate_description(input_col, metadata)
    ...
    if isinstance(feature, ft.feature_base.FeatureOutputSlice):
        feature = feature.base_feature  # use base of multi-output feature for the rest of the description
    ...
    if isinstance(feature, ft.AggregationFeature):
        # of all 'sessions'
        # of the previous 5 days of 'sessions'"
        if feature.use_previous:
            entity_description = "of the previous {} of ".format(feature.use_previous.get_name().lower())
        else:
            entity_description = "of all "
        # entity = try_to_make_plural(feature.relationship_path[-1][1].child_entity.id)
        entity = '"{}"'.format(feature.relationship_path[-1][1].child_entity.id)
        entity_description += "instances of {}".format(entity)
    ...
    if hasattr(feature, 'where') and feature.where:
        ...
        if feature.where in feature_descriptions or feature.where.unique_name() in feature_descriptions:
            ...
            where_col = generate_description(feature.where.base_features[0], metadata)
        where = "where {} is {}".format(where_col, where_value)

    # 8) join parts of description
...
def parse_metadata_json(metadata_file, allow_functions_from_json):
    pass
