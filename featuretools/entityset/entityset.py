    ...
    def add_dataframe(
        ...
        if dataframe.ww.schema is None:
            ...
            if not set(dataframe.columns).issubset(set(logical_types.keys())) and (
                ...
                or is_instance(dataframe, ks, "DataFrame")
            ):
                warnings.warn(
                    "Performing type inference on Dask or Koalas DataFrames may be computationally intensive. "
        ...
        while not to_explore.issubset(explored):
            ...
            if dataframe.ww.name in children:
                ...
                for child_df in child_dataframes:
                    ...
                    lti_is_koalas = is_instance(
                        es_lti_dict[child_df.ww.name], ks, "Series"
                    )
                    ...
                    if not (lti_is_dask or lti_is_koalas) and lti_df.empty:
                        ...
                        if lti_is_koalas:
                            ...
                            lti_df["last_time_old"] = ks.to_datetime(
    ...
    def query_by_values(
        ...
        if instance_vals is None:
            ...
            if is_instance(instance_vals, (dd, ks), "Series"):
                ...
                dataframe, ks, "DataFrame"
            ):
                df = dataframe.merge(
                    ks.DataFrame({column_name: instance_vals}),
                    how="inner",
                    on=column_name,
                )
            else:
