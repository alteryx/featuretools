...
class RollingPrimitiveUtils:
    ...
    def roll_series_with_gap(series, window_size, gap=0, min_periods=1):
        ...
        """
        RollingPrimitiveUtils._check_window_size(window_size)
        RollingPrimitiveUtils._check_gap(window_size, gap)
    ...
    def apply_roll_with_offset_gap(window, gap_offset, reducer_fn, min_periods):
        ...
        """
        window = RollingPrimitiveUtils._get_rolled_series_without_gap(
            window, gap_offset
        )
    ...
    def _check_window_size(window_size):
        ...
        # Or an integer greater than zero
            ...
            raise TypeError(
                "Window length must be either an offset string or an integer."
            )
